...
format:
  revealjs:
    ...
    logo: img/logos/png/logoMASA.png   # Add this line
...
library(tidyverse)
library(echarts4r)
d2 <- readRDS("iaa_entre_bongroupe.rds") 

D_graph <- d2 %>%  mutate(acti3 = substr(ape_diff,1,3)) %>% filter(acti3 %in% c(101,102,103,104,105,106,107,108,109,110,462,463)) %>%   group_by(acti3,classe) %>% summarise(across(where(is.numeric),\(x) round(sum(x,na.rm = TRUE)/1000000,4),.names = "{col}")) %>% 
  group_by(acti3) %>% mutate(across(where(is.numeric),\(x) x/sum(x,na.rm=TRUE),.names="{col} en %")) %>% as.data.frame()


D_graph1 <- D_graph %>% select(acti3,classe,redi_r310) %>%  pivot_wider(names_from = classe,values_from = redi_r310)
...
rownames(D_graph1) <- D_graph1$acti3
 ...
 graph <- D_graph1 %>%
  ...
  e_bar(`tête de groupe étrangère`, name = "non coopérative:étrangère", itemStyle = list(color = "#1E3A5F")) %>% 
  e_bar(total, itemStyle = list(color = "#D3D3D3"),
         label = list(show = TRUE, position = "top", fontSize = 10, color = "black")) %>% 
  e_x_axis(name = "Activités") %>%  # Nom de l'axe X
  e_y_axis(name = "Chiffre d'affaires en millard d'euro") %>%  # Nom de l'axe Y
  e_tooltip() %>% 
  e_toolbox(right = "5%") %>%  # Toolbox plus à gauche
  ...
  e_legend(selected = list(
    ...
    "non coopérative:étrangère" = FALSE,
 ...
 graph <- onRender(graph, "
  function(el, x) {
    el.style.width = '65vw';
    el.style.height = '50vh';
    el.style.position = 'absolute';
    el.style.top = '1Ovh';   // Décale le graphique vers le bas
    el.style.left = '-10vw';   // Décale le graphique vers la gauche
    
    
    // Empêcher le graphique de devenir trop grand
